...
- Beating the time-trial target unlocks a bright yellow golf ball. Inspecting verified experimental structures for two distinct human proteins unlocks a white ball with blue/red/blue alignment stripes. White and yellow have different personal stories; striped launches six multiple-choice bioinformatics questions with seven seconds to answer after the ball arrives. Correct answers and explanations remain for review after an answer or timeout. Untimed play and a readable study list are available. Each new reward opens a dismissible popup with an option to select it. Story completion, best trial score, discoveries, unlocks, ball selection, and dismissed popups persist locally; blocked storage falls back to the current session.
