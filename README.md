...
- The selected protein always shows a fixed-height description excerpt and a link to its full STRING record. Short or unavailable descriptions preserve the same space. The network fills its panel without stretching an empty area below its controls. Beside experimental structure evidence, a compact implementation panel explains source integration, data processing, grounded AI, and reliability choices; the expandable data view retains keyboard protein selection and provenance.
