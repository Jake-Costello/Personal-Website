# Jake Costello — Personal Website
...
- Responsive portfolio, two Selected Work case studies (Revision Marine and the live protein explorer), project detail dialogs, about content, and contact links. The payphone senior project remains in the resume timeline. Revision Marine identifies Jake as its founding engineer, describes his software/infrastructure and design responsibilities, and links to its currently private storefront preview from the project and timeline.
...
- An open about layout with the introduction on the left and an isolated pixel golfer on a small grass tee on the right. Six clubs in the bag reveal stories about animation to computer science, climbing, beach volleyball, drawing, and golf, plus a putter joke. Hover highlights a club; drag it to the golfer or activate it with a tap/keyboard. A white, dimpled ball flies beyond the artwork to fill the viewport, reveals the fact for seven seconds, then falls away. Keep reading pauses the story; reduced motion also keeps stories open until dismissed.
- The selected protein always shows a fixed-height description excerpt and a link to its full STRING record. Short or unavailable descriptions preserve the same space. The network fills its panel without stretching an empty area below its controls. Beside experimental structure evidence, a compact implementation panel explains source integration, data processing, grounded AI, and reliability choices; graph nodes support keyboard selection, and a compact source footer retains provenance.
- Beating the time-trial target unlocks a bright yellow golf ball. Inspecting verified experimental structures for two distinct human proteins unlocks a white ball with blue/red/blue alignment stripes. White and yellow have different personal stories; striped launches six multiple-choice bioinformatics questions with seven seconds to answer after the ball arrives. Correct answers and explanations remain for review after an answer or timeout. Each new reward opens a dismissible popup with an option to select it. Story completion, best trial score, discoveries, unlocks, ball selection, and dismissed popups persist locally; blocked storage falls back to the current session.
...
- On-demand experimental-structure evidence from RCSB PDB for all twelve catalog proteins. Entry and polymer-entity records are checked for experimental methodology, human source organism, and the expected UniProt identifier. The panel shows the method, available resolution, source, and retrieval time, then counts a unique discovery. Failed, predicted, mismatched, or abandoned results do not award progress. These are public browser requests and require no API key or extra hosting.
...
See [editing golf stories and achievement messages](docs/GOLF-CONTENT.md) for the per-ball text fields, unlock popup wording, and challenge behavior. Experimental-structure mappings live in `src/data/proteins.ts`; they are curated examples whose evidence is rechecked against RCSB when inspected.
