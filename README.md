...
- Beating the time-trial target unlocks a bright yellow golf ball. Inspecting verified experimental structures for two distinct human proteins unlocks a white ball with blue/red/blue alignment stripes. White, yellow, and striped each have six different stories, reflected in the clubs, flying fact ball, and readable list. Each new reward opens a dismissible popup with an option to select it. Story completion, best trial score, discoveries, unlocks, ball selection, and dismissed popups persist locally; blocked storage falls back to the current session.
