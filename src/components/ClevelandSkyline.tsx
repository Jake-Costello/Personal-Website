...
const FLYOVER_DURATION_MS = 24_000;
...
function Blimp({
  ...
  useEffect(() => {
    const remaining = FLYOVER_DURATION_MS - (performance.now() - startedAt);
    if (remaining <= 0) {
      setElapsed(FLYOVER_DURATION_MS);
      return;
    }
    // Reduced motion gets the same one-time appearance without a moving backdrop.
    if (reduced) {
      const timer = window.setTimeout(() => setElapsed(FLYOVER_DURATION_MS), remaining);
      return () => window.clearTimeout(timer);
    }
    let frame: number;
    let lastPaint = 0;
    function animate(now: number) {
      const age = Math.min(FLYOVER_DURATION_MS, now - startedAt);
      if (now - lastPaint >= 1000 / 30 || age === FLYOVER_DURATION_MS) {
        setElapsed(age);
        lastPaint = now;
      }
      if (age < FLYOVER_DURATION_MS) frame = requestAnimationFrame(animate);
    }
    frame = requestAnimationFrame(animate);
    return () => cancelAnimationFrame(frame);
  }, [startedAt, reduced]);

  if (elapsed >= FLYOVER_DURATION_MS) return null;
  const blimpWidth = 177 * scale;
  const x = reduced
    ? Math.max(12, width - blimpWidth - (width < 560 ? 24 : 120))
    : width + 24 - (width + 48 + blimpWidth) * (elapsed / FLYOVER_DURATION_MS);
  return (
    <g
      className="skyline-blimp"
  ...
  return (
    <>
      {blimpStartedAt !== null && (
        <Blimp
          width={width}
          y={waterline + blimpY * scale}
          scale={scale * (width < 560 ? 1.04 : 0.95)}
          startedAt={blimpStartedAt}
          reduced={reduced}
        />
      )}
      <g
      ...
      >
        ...
        <g transform="translate(273 0)">
          ...
          />
          <path
            d="M135-157h8v139h-8zm17 14h7v125h-7zm-35 43h7v82h-7zm54 0h7v82h-7z"
            fill="#a7c0b1"
          />
        </g>
